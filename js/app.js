...
function generateSummary(tasks, subjects) {
  ...
  const topSubject = Object.keys(subjectCount).length
    ? Object.keys(subjectCount).reduce((a, b) => subjectCount[a] > subjectCount[b] ? a : b)
    : 'no specific subject';
...
/**
 * Returns the HTML string for a priority badge.
 * @param {'low'|'medium'|'high'|string} priority
 */
function priorityBadgeHtml(priority) {
  const map = {
    low:    { cls: 'priority-badge-low',    icon: '🟢', label: 'Low'    },
    medium: { cls: 'priority-badge-medium', icon: '🟡', label: 'Medium' },
    high:   { cls: 'priority-badge-high',   icon: '🔴', label: 'High'   },
  };
  const p = map[priority] || map['medium'];
  return `<span class="priority-badge ${p.cls}" title="Priority: ${p.label}">${p.icon} ${p.label}</span>`;
}
...
let currentView      = 'calendar'; // 'calendar' | 'all-tasks' | 'archived' | 'focus'
...
function renderSidebarSubjects() {
  ...
  subjects.forEach(s => { countBySubject[s.id] = 0; });
  tasks.forEach(t => {
...
// ── Priority button group wiring ──
const priorityGroup    = document.getElementById('new-task-priority-group');
const priorityHidden   = document.getElementById('new-task-priority');
...
if (priorityGroup) {
  priorityGroup.addEventListener('click', e => {
    ...
    priorityHidden.value = btn.dataset.value;
  });
}

function resetPriorityGroup(defaultValue = 'low') {
  if (!priorityGroup) return;
  priorityGroup.querySelectorAll('.priority-btn').forEach(b => {
    b.classList.toggle('priority-btn-active', b.dataset.value === defaultValue);
  });
  if (priorityHidden) priorityHidden.value = defaultValue;
}
...
function calculateTimeFraction() {
  const raw = timeLeft / TIME_LIMIT;
  return raw - (1 / TIME_LIMIT) * (1 - raw);
}

function setCircleDasharray() {
  timerPathRemaining.setAttribute('stroke-dasharray',
...
function renderFocusTasks() {
  ...
  const dueSoon = tasks
    ...
    .filter(t => (new Date(t.due_at) - now) / (1000 * 60 * 60 * 24) <= 3)
  ...
  if (activeFocusTaskId) {
    ...
    if (activeT) {
      ...
      activeFocusTask.innerHTML = `
        <div class="task-info" style="width:100%">
          ...
          <div class="task-meta">
            ...
            ${priorityBadgeHtml(activeT.priority || 'medium')}
          </div>
      ...
      activeFocusTask.querySelector('.complete-focus-task-btn')?.addEventListener('click', () => {
        store.toggleTaskStatus(activeT.id);
        activeFocusTaskId = null;
        renderFocusTasks();
      });
      activeFocusTask.querySelector('.clear-focus-task-btn')?.addEventListener('click', () => {
        activeFocusTaskId = null;
        renderFocusTasks();
      });
    } else {
...
function renderTasks() {
  ...
  if (currentView === 'calendar' && selectedDate) {
    sorted.forEach(t => {
      ...
      if (d.getDate() === selectedDate.getDate() &&
        ...
        else { dueSoon.push(t); pending.push(t); }
      }
    });
  } else {
    sorted.forEach(t => {
      ...
      if (diff <= 3) dueSoon.push(t);
      else thisWeek.push(t);
    });
  }

  // ── Inline edit form for a task ──
  const editFormHtml = (t) => {
    ...
    const curPriority = t.priority || 'medium';
    ...
    return `
      <div class="task-item" style="display:block; padding:12px; cursor:default;" data-id="${t.id}">
        ...
        <label style="display:block; font-size:10px; font-weight:700; color:var(--color-text-tertiary); text-transform:uppercase; letter-spacing:0.04em; margin-bottom:6px;">Priority</label>
        <div class="priority-btn-group board-edit-priority-group" style="margin-bottom:12px;">
          <button type="button" class="priority-btn priority-btn-low   ${curPriority === 'low'    ? 'priority-btn-active' : ''}" data-value="low">🟢 Low</button>
          <button type="button" class="priority-btn priority-btn-medium ${curPriority === 'medium' ? 'priority-btn-active' : ''}" data-value="medium">🟡 Medium</button>
          <button type="button" class="priority-btn priority-btn-high  ${curPriority === 'high'   ? 'priority-btn-active' : ''}" data-value="high">🔴 High</button>
        </div>
        <input type="hidden" class="board-edit-priority-val" value="${curPriority}">
  ...
  const taskCardHtml = (t) => {
    ...
    return `
      <div class="task-item ${isUrgent && !isDone ? 'urgent' : ''} ${isDone ? 'done' : ''}" data-id="${t.id}">
  ...
  const renderGroup = (title, items, titleColor, showConflict = false) => {
    ...
    let html = `<div class="tasks-group">
      <div class="tasks-group-header"><span style="color:${titleColor}">${title}</span></div>`;
    ...
    if (showConflict) {
      ...
      workloadSuggestions.forEach(workload => {
        html += `<div class="conflict-card smart-workload-card ${workload.level}">
          ...
          <ul class="smart-suggestion-list">
            ${workload.suggestions.map(s =>
              `<li class="${s.includes('Suggested reschedule') ? 'smart-highlight' : ''}">${s}</li>`
            ).join('')}
          </ul>
        </div>`;
      });
    ...
    items.forEach(t => {
      html += t._isEditing ? editFormHtml(t) : taskCardHtml(t);
    });
    html += `</div>`;
    return html;
  };
  ...
  if (currentView === 'calendar' && selectedDate) {
    ...
    const emptyStateText = currentView === 'archived'
      ? 'No archived tasks.'
      : 'No tasks yet. Add tasks from Smart Paste to get started.';
  ...
  // ── Wire up inline-edit priority button groups ──
  document.querySelectorAll('.board-edit-priority-group').forEach(group => {
    group.addEventListener('click', e => {
      const btn = e.target.closest('.priority-btn');
      if (!btn) return;
      group.querySelectorAll('.priority-btn').forEach(b => b.classList.remove('priority-btn-active'));
      btn.classList.add('priority-btn-active');
      // write to hidden sibling
      const hiddenInput = group.nextElementSibling;
      if (hiddenInput && hiddenInput.classList.contains('board-edit-priority-val')) {
  ...
  document.querySelectorAll('.edit-task-btn').forEach(el => {
    el.addEventListener('click', e => { e.stopPropagation(); store.setTaskEditing(el.dataset.id, true); });
  });

  document.querySelectorAll('.cancel-board-edit-btn').forEach(el => {
    el.addEventListener('click', e => { e.stopPropagation(); store.setTaskEditing(el.dataset.id, false); });
  });
  ...
  document.querySelectorAll('.save-board-edit-btn').forEach(el => {
    el.addEventListener('click', e => {
      ...
      const dateVal    = itemEl.querySelector('.board-edit-date').value;
      const notes      = itemEl.querySelector('.board-edit-notes').value;
      const priority   = itemEl.querySelector('.board-edit-priority-val').value;
  ...
  document.querySelectorAll('.task-check').forEach(el => {
    el.addEventListener('click', e => {
      e.stopPropagation();
      store.toggleTaskStatus(el.closest('.task-item').dataset.id);
    });
  });

  document.querySelectorAll('.archive-task-btn').forEach(el => {
    el.addEventListener('click', e => { e.stopPropagation(); store.archiveTask(el.dataset.id); });
  });

  document.querySelectorAll('.restore-task-btn').forEach(el => {
    el.addEventListener('click', e => { e.stopPropagation(); store.restoreTask(el.dataset.id); });
  });

  document.querySelectorAll('.delete-task-btn').forEach(el => {
    el.addEventListener('click', e => { e.stopPropagation(); store.deleteTask(el.dataset.id); });
  });

  const markAllPendingBtn = document.getElementById('mark-all-pending-btn');
  if (markAllPendingBtn) {
    markAllPendingBtn.addEventListener('click', e => { e.stopPropagation(); store.markAllPendingCompleted(); });
  }

  const markDayCompleteBtn = document.getElementById('mark-day-complete-btn');
  if (markDayCompleteBtn) {
    markDayCompleteBtn.addEventListener('click', e => { e.stopPropagation(); store.markPendingTasksForDateCompleted(selectedDate); });
  }
}
...
if (summaryBox) summaryBox.innerHTML = generateSummary(store.tasks, store.subjects);
...
function renderCalendar() {
  ...
  for (let i = 1; i <= daysInMonth; i++) {
    ...
    html += `<div class="cal-day interactive-day ${isToday ? 'today' : ''}" data-day="${i}" style="${isSelected ? 'border:1.5px solid var(--color-text-primary);' : ''}">
      ${i}${indicatorHtml}
    </div>`;
  }
  ...
  for (let i = 1; i <= nextDays; i++) html += `<div class="cal-day muted">${i}</div>`;
  ...
  document.querySelectorAll('.interactive-day').forEach(el => {
    el.addEventListener('click', e => {
      ...
      selectedDate = (selectedDate && clickedDate.getTime() === selectedDate.getTime()) ? null : clickedDate;
      renderCalendar();
      renderTasks();
    });
  });
...
function renderExtraction() {
  ...
  pasteItems.forEach((item, index) => {
    ...
    if (item._isEditing) {
      const subjectOptions = store.subjects.map(s =>
      ...
      const curPriority = item.priority || 'medium';

      html += `
        <div class="extract-card">
          <label style="display:block; font-size:10px; font-weight:700; color:var(--color-text-tertiary); text-transform:uppercase; letter-spacing:0.04em; margin-bottom:4px;">Subject</label>
          <select class="edit-subject-input" data-index="${index}" style="width:100%; margin-bottom:12px; font-size:12px; padding:4px; border:1px solid var(--color-border-secondary); border-radius:4px; background:var(--color-background-primary); color:var(--color-text-primary);">
            ${subjectOptions}
          </select>
          <label style="display:block; font-size:10px; font-weight:700; color:var(--color-text-tertiary); text-transform:uppercase; letter-spacing:0.04em; margin-bottom:4px;">Task Name</label>
          <input class="edit-title-input" type="text" value="${escapeHtml(item.title)}" data-index="${index}" style="width:100%; margin-bottom:12px; font-size:13px; font-weight:600; padding:6px; border:1px solid var(--color-border-secondary); border-radius:4px; background:var(--color-background-primary); color:var(--color-text-primary);">
          <label style="display:block; font-size:10px; font-weight:700; color:var(--color-text-tertiary); text-transform:uppercase; letter-spacing:0.04em; margin-bottom:4px;">Deadline</label>
          <input class="edit-date-input" type="datetime-local" value="${localDate}" data-index="${index}" style="width:100%; margin-bottom:12px; font-size:12px; padding:6px; border:1px solid var(--color-border-secondary); border-radius:4px; background:var(--color-background-primary); color:var(--color-text-primary);">
          <label style="display:block; font-size:10px; font-weight:700; color:var(--color-text-tertiary); text-transform:uppercase; letter-spacing:0.04em; margin-bottom:4px;">Notes</label>
          <input class="edit-notes-input" type="text" value="${escapeHtml(item.notes || '')}" data-index="${index}" placeholder="Notes..." style="width:100%; margin-bottom:12px; font-size:12px; padding:6px; border:1px solid var(--color-border-secondary); border-radius:4px; background:var(--color-background-primary); color:var(--color-text-primary);">
          <label style="display:block; font-size:10px; font-weight:700; color:var(--color-text-tertiary); text-transform:uppercase; letter-spacing:0.04em; margin-bottom:6px;">Priority</label>
          <div class="priority-btn-group extract-edit-priority-group" style="margin-bottom:12px;">
            <button type="button" class="priority-btn priority-btn-low    ${curPriority === 'low'    ? 'priority-btn-active' : ''}" data-value="low">🟢 Low</button>
            <button type="button" class="priority-btn priority-btn-medium ${curPriority === 'medium' ? 'priority-btn-active' : ''}" data-value="medium">🟡 Medium</button>
            <button type="button" class="priority-btn priority-btn-high   ${curPriority === 'high'   ? 'priority-btn-active' : ''}" data-value="high">🔴 High</button>
          </div>
          <input type="hidden" class="extract-edit-priority-val" value="${curPriority}">
          <div style="display:flex; justify-content:flex-end; gap:8px; margin-top:4px;">
      ...
      html += `
        <div class="extract-card" style="animation-delay:${index * 0.1}s">
          ...
          <div class="extract-row" style="margin-top:6px;">${priorityBadgeHtml(item.priority || 'medium')}</div>
  ...
  setTimeout(() => {
    document.querySelectorAll('.conf-fill').forEach(el => { el.style.width = el.getAttribute('data-width') + '%'; });
  }, 100);

  // Wire priority groups inside extract edit cards
  document.querySelectorAll('.extract-edit-priority-group').forEach(group => {
    group.addEventListener('click', e => {
      const btn = e.target.closest('.priority-btn');
      if (!btn) return;
      group.querySelectorAll('.priority-btn').forEach(b => b.classList.remove('priority-btn-active'));
      btn.classList.add('priority-btn-active');
      const hiddenInput = group.nextElementSibling;
      if (hiddenInput && hiddenInput.classList.contains('extract-edit-priority-val')) {
        hiddenInput.value = btn.dataset.value;
      }
    });
  });
  ...
  document.querySelectorAll('.conf-edit').forEach(btn => {
    btn.addEventListener('click', e => {
      store.updateExtractedItem(e.target.getAttribute('data-index'), { _isEditing: true });
    });
  });
  ...
  document.querySelectorAll('.save-edit-btn').forEach(btn => {
    btn.addEventListener('click', e => {
      ...
      const priority  = card.querySelector('.extract-edit-priority-val').value;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (newSubjectColorsEl) {
    SUBJECT_COLORS.forEach(c => {
      ...
      btn.addEventListener('click', () => { selectedNewSubjectColor = c; syncNewSubjectColorSwatches(); });
  ...
  if (addSubjectBtn) {
    ...
    addSubjectBtn.addEventListener('keydown', e => {
      if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); openNewSubjectModal(); }
    });
  }

  if (newSubjectCancel) newSubjectCancel.addEventListener('click', () => { if (newSubjectModal) newSubjectModal.style.display = 'none'; });
  if (newSubjectModal)  newSubjectModal.addEventListener('click', e => { if (e.target === newSubjectModal) newSubjectModal.style.display = 'none'; });
  if (newSubjectSave)   newSubjectSave.addEventListener('click', async () => {
  ...
  if (newSubjectName) newSubjectName.addEventListener('keydown', e => { if (e.key === 'Enter') { e.preventDefault(); newSubjectSave?.click(); } });
  ...
  document.getElementById('cal-prev').addEventListener('click', () => { currentMonthDate.setMonth(currentMonthDate.getMonth() - 1); renderCalendar(); });
  document.getElementById('cal-next').addEventListener('click', () => { currentMonthDate.setMonth(currentMonthDate.getMonth() + 1); renderCalendar(); });
  ...
  newTaskBtn.addEventListener('click', () => {
    ...
    newTaskSubject.innerHTML = store.subjects.map(s => `<option value="${s.id}">${s.name}</option>`).join('');
    if (selectedDate) {
    ...
    newTaskTitle.value = '';
    newTaskNotes.value = '';
    resetPriorityGroup('low'); // default to Low when opening
    newTaskModal.style.display = 'flex';
  });

  newTaskCancel.addEventListener('click', () => { newTaskModal.style.display = 'none'; });
  newTaskModal.addEventListener('click', e => { if (e.target === newTaskModal) newTaskModal.style.display = 'none'; });
  ...
  newTaskSave.addEventListener('click', async () => {
    ...
    const priority   = priorityHidden ? priorityHidden.value : 'medium';
...
// ─────────────────────────────────────────────
// Extract / Clear / Download (outside DOMContentLoaded — safe, buttons exist)
// ─────────────────────────────────────────────
