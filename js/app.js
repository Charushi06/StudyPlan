...
function setCircleDasharray() {
  timerPathRemaining.setAttribute(
    'stroke-dasharray',
...
function renderFocusTasks() {
  ...
  if (dueSoon.length === 0) {
    ...
    focusTaskList.innerHTML = dueSoon.map(t => {
      ...
      return `<div class="focus-task-item" data-id="${t.id}">
        <div class="task-name">${t.title}</div>
        <div class="task-meta"><span class="task-pill ${pillClass}">${sub.short_code}</span></div>
      </div>`;
    }).join('');
...
function renderTasks() {
  ...
  if (currentView === 'calendar' && selectedDate) {
    ...
    sorted.forEach(t => {
      ...
      pending.push(t);
      const diffDays = (new Date(t.due_at) - now) / (1000 * 60 * 60 * 24);
  ...
  const renderGroup = (title, items, titleColor, showConflict = false) => {
    ...
    if (showConflict) {
      analyzeWorkload(items).forEach(workload => {
        html += `<div class="conflict-card smart-workload-card ${workload.level}">
          ...
          <ul class="smart-suggestion-list">${workload.suggestions.map(s =>
    ...
    items.forEach(t => {
      ...
      if (t._isEditing) {
        ...
        html += `<div class="task-item" style="display:block; padding:12px; cursor:default;" data-id="${t.id}">
          ...
          <select class="board-edit-subject edit-field" style="width:100%; margin-bottom:12px; font-size:12px; padding:4px; border:1px solid var(--color-border-secondary); border-radius:4px; background:var(--color-background-primary); color:var(--color-text-primary);">${subjectOptions}</select>
          ...
          <div style="display:flex; justify-content:flex-end; gap:8px; margin-top:4px;">
            <button class="btn cancel-board-edit-btn" data-id="${t.id}" style="padding:6px 12px; font-size:11px;">Cancel</button>
        ...
        const archiveBtn = !t.archived
          ? `<button class="task-btn edit-task-btn" data-id="${t.id}">&#9999;&#65039; Edit</button>
             <button class="task-btn archive-task-btn" data-id="${t.id}">Archive</button>`
          : `<button class="task-btn edit-task-btn" data-id="${t.id}">&#9999;&#65039; Edit</button>
             <button class="task-btn task-btn-info restore-task-btn" data-id="${t.id}">Restore</button>
             <button class="task-btn task-btn-danger delete-task-btn" data-id="${t.id}">Delete</button>`;

        html += `<div class="task-item ${isUrgent ? 'urgent' : ''} ${isDone ? 'done' : ''}" data-id="${t.id}">
  ...
  document.querySelectorAll('.task-item').forEach(el => {
    el.addEventListener('click', (e) => {
      ...
      const task = store.tasks.find(t => String(t.id) === String(el.dataset.id));
  ...
  document.querySelectorAll('.save-board-edit-btn').forEach(el => {
    el.addEventListener('click', (e) => {
      ...
      store.updateTask(el.dataset.id, {
        title:      itemEl.querySelector('.board-edit-title').value,
        subject_id: itemEl.querySelector('.board-edit-subject').value,
        due_at:     itemEl.querySelector('.board-edit-date').value
                      ? new Date(itemEl.querySelector('.board-edit-date').value).toISOString() : '',
        notes:      itemEl.querySelector('.board-edit-notes').value,
        priority:   itemEl.querySelector('.board-edit-priority').value,
      });
    });
  });
  ...
  document.getElementById('mark-all-pending-btn')?.addEventListener('click', (e) => {
    e.stopPropagation(); store.markAllPendingCompleted();
  });
  document.getElementById('mark-day-complete-btn')?.addEventListener('click', (e) => {
    e.stopPropagation(); store.markPendingTasksForDateCompleted(selectedDate);
  });
}
...
function renderCalendar() {
  ...
  for (let i = 1; i <= daysInMonth; i++) {
    ...
    html += `<div class="cal-day interactive-day ${isToday ? 'today' : ''}" data-day="${i}" style="${extraStyle}">${i}${indicatorHtml}</div>`;
  }
...
function renderExtraction() {
  ...
  let html = `<div class="extract-title">Extracted &mdash; ${pasteItems.length} items</div>`;
  pasteItems.forEach((item, index) => {
    ...
    if (item._isEditing) {
      ...
      html += `<div class="extract-card">
        <label style="display:block; font-size:10px; font-weight:700; color:var(--color-text-tertiary); text-transform:uppercase; letter-spacing:0.04em; margin-bottom:4px;">Subject</label>
        <select class="edit-subject-input edit-field" data-index="${index}" style="width:100%; margin-bottom:12px; font-size:12px; padding:4px; border:1px solid var(--color-border-secondary); border-radius:4px; background:var(--color-background-primary); color:var(--color-text-primary);">${subjectOptions}</select>
        <label style="display:block; font-size:10px; font-weight:700; color:var(--color-text-tertiary); text-transform:uppercase; letter-spacing:0.04em; margin-bottom:4px;">Task Name</label>
      ...
      html += `<div class="extract-card" style="animation-delay:${index * 0.1}s">
  ...
  document.querySelectorAll('.save-edit-btn').forEach(btn => {
    btn.addEventListener('click', (e) => {
      ...
      store.updateExtractedItem(idx, {
        ...
        notes:        card.querySelector('.edit-notes-input').value,
        _isEditing:   false,
      });
    });
  });
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  newSubjectName?.addEventListener('keydown', (e) => {
    if (e.key === 'Enter') { e.preventDefault(); newSubjectSave?.click(); }
  });
  ...
  newTaskSave.addEventListener('click', async () => {
    ...
    await store.addTasks([{
      title,
      subject_id: newTaskSubject.value,
      due_at:     dateVal ? new Date(dateVal).toISOString() : '',
...
extractBtn.addEventListener('click', async () => {
  ...
  extractBtn.innerHTML = 'Extract with AI &rarr;';
