...
function generateSummary(tasks, subjects) {
  ...
  let completedDayCount = {};

  tasks.forEach(t => {
    if (t.archived || !t.due_at) return;
    ...
    if (t.status === 'Done') {
      ...
      completedDayCount[dayName] =
        (completedDayCount[dayName] || 0) + 1;
    }
  ...
  const productiveDay = Object.keys(completedDayCount).length
    ? Object.keys(completedDayCount).reduce((a, b) =>
      completedDayCount[a] > completedDayCount[b] ? a : b
    )
    : 'No active day';

  const activeDays =
    Object.keys(completedDayCount).length;
  ...
  return `
    ...
    Completed tasks: <b>${completedThisWeek}</b><br>
    Most productive day: <b>${productiveDay}</b><br>
    Consistency: <b>${consistency}%</b>
  `;
}
