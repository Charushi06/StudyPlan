...
function renderNotes() {
  ...
  notesList.innerHTML = filtered.map(note => `
    <div class="note-card" style="padding: 16px; border: 1px solid var(--color-border-secondary);
                                  border-radius: 8px; cursor: pointer; transition: all 0.2s;">
      <h3 style="margin: 0 0 8px; font-size: 16px; font-weight: 600; color: var(--color-text-primary);">
        ${note.title}
      </h3>
      <p style="margin: 0 0 12px; font-size: 13px; color: var(--color-text-secondary);
                line-height: 1.5; max-height: 60px; overflow: hidden; text-overflow: ellipsis; white-space: pre-wrap;">
      ...
      <div style="display: flex; justify-content: space-between; align-items: center; font-size: 12px; color: var(--color-text-secondary);">
        ...
        <button class="note-edit-btn" data-id="${note.id}" style="padding: 4px 8px; background: var(--color-background-secondary); border: 1px solid var(--color-border-secondary); border-radius: 4px; cursor: pointer;">
          Edit
        </button>
      </div>
    </div>
  ...
  document.querySelectorAll('.note-card').forEach(card => {
    card.addEventListener('click', () => {
      const noteId = card.querySelector('.note-edit-btn').dataset.id;
...
function updateAllViews() {
  ...
  document.querySelector('.cal-section').style.display = 'none';
  ...
  if (currentView === 'calendar') {
    ...
    document.querySelector('.cal-section').style.display = 'block';
    ...
    document.getElementById('archived-tasks-btn').classList.add('active');
    document.getElementById('tasks-section').style.display = 'block';
    renderTasks();
...
if (dropZone) {
  ...
  const newNoteSaveBtn = document.getElementById('new-note-save');
  if (newNoteSaveBtn) {
    newNoteSaveBtn.addEventListener('click', async () => {
      ...
      const success = await store.addNote({ title, content });
      if (success) {
        closeNewNoteModal();
        renderNotes();
      }
  ...
  const newNoteCancelBtn = document.getElementById('new-note-cancel');
  if (newNoteCancelBtn) {
    newNoteCancelBtn.addEventListener('click', closeNewNoteModal);
  }

  const editNoteSaveBtn = document.getElementById('edit-note-save');
  if (editNoteSaveBtn) {
    editNoteSaveBtn.addEventListener('click', async () => {
      ...
      await store.updateNote(currentEditingNoteId, { title, content });
      closeEditNoteModal();
      renderNotes();
    });
  }

  const editNoteCancelBtn = document.getElementById('edit-note-cancel');
  if (editNoteCancelBtn) {
    editNoteCancelBtn.addEventListener('click', closeEditNoteModal);
  }

  const editNoteDeleteBtn = document.getElementById('edit-note-delete');
  if (editNoteDeleteBtn) {
    editNoteDeleteBtn.addEventListener('click', async () => {
      await store.deleteNote(currentEditingNoteId);
      closeEditNoteModal();
      renderNotes();
    });
  }
